...
test.describe('Flipkart Seach via the SVG', () => {
    ...
    test('TC#1 @smoke @regression', async ({ page }) => {
        ...
        const price: Locator = page.locator("//div[contains(@data-id,'CPU') or contains(@data-id,'ACC') or contains(@data-id,'COM') or contains(@data-id,'MP')]/div/a[3]");
        const priceArray: string[] = [];
        const count1: number = await price.count();
        for (let i = 0; i < count1; i++) {
            const priceTag: string | null = await price.nth(i).textContent();
            console.log(priceTag);
            if (typeof (priceTag) === "string")
                priceArray.push(priceTag);
        }

        const sortPriceTag = priceArray.sort();
        console.log("Lowest Price:", sortPriceTag[0]);
