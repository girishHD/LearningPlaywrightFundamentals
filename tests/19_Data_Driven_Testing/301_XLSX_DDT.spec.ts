...
function fetchUsersFromXLSX(): LoginData[] {
    const workbook = XLSX.readFile(path.join(__dirname, 'test-data/login-data.xlsx'));
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    return XLSX.utils.sheet_to_json(sheet) as LoginData[];
}

const loginData = fetchUsersFromXLSX();
